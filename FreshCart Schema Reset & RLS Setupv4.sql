...
create table public.store_products (
  ...
  -- v3: non-negative constraint prevents overselling
...
create table public.cart_items (
  ...
  -- v3: store_id stored directly — eliminates extra round-trip at checkout
  --     and enables multi-store guard. Auto-set by trg_set_cart_item_store.
  store_id         uuid references public.stores(id),
...
create table public.reviews (
  id               uuid primary key default gen_random_uuid(),
  -- customer_id kept for backward compat; reviewer_id is the canonical FK
  customer_id      uuid references public.profiles(id),
  -- v3: reviewer_id used in insert RLS policy
...
-- Atomic stock decrement — raises exception if insufficient stock.
-- Called from the app via supabase.rpc('decrement_stock', {...})
...
-- so the app never has to look it up separately.
...
-- v3: customers can only review products from delivered orders
...
-- v3: enables fast multi-store check query
create index idx_cart_items_customer_store   on public.cart_items(customer_id, store_id);
...
-- v3: fast lookup for pending deletion requests (admin dashboard)
create index idx_profiles_deletion_requested on public.profiles(deletion_requested)
...
-- =====================================================
-- STEP 12: REALTIME (uncomment when ready)
-- =====================================================
